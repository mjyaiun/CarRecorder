package com.guotiny.okhttpserver.download;
...
import com.guotiny.okhttpserver.L;
import com.guotiny.okhttpserver.listener.DownloadListener;
import com.guotiny.okhttpserver.task.PriorityAsyncTask;
...
public class DownloadTask extends PriorityAsyncTask<Void, DownloadInfo, DownloadInfo> {
    ...
    protected DownloadInfo doInBackground(Void... params) {
        ...
        try {
            ...
            header.put("RANGE", "bytes=" + startPos + "-");
            response = OkHttpUtils.get().url(url).headers(header).build().execute();
